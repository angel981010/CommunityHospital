...
//    暂时注释掉，改成静态数据wangyao 0828
    if (self.commlistArray!=nil) {
        return [self.commlistArray count];
    }else{
        return 0;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.commlistArray!=nil){
        CommodityInfo *comm_info = [self.commlistArray objectAtIndex:indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Detail4Food2ViewController *DFVC = [Detail4Food2ViewController createFromStoryboardName:@"Detail4Food2" withIdentifier:@"detail_4_food_2"];
...
- (void)viewDidLoad {
    ...
    [self.hud show:YES];
    //公告如果是空的就不显示了
    if (![self.shop_notice isEqualToString:@""]&&self.shop_notice!=nil) {
    ...
    FirstLoad = true;
    //导航栏
    if (RecommendCommodity) {
        self.navigationItem.title = @"推荐商品列表";
    }else{
    ...
    [CommodityList LoadCommodityListWithShopID:self.shop_id page:[NSNumber numberWithInt:1] rows:[NSNumber numberWithInt:20] Success:^(id object) {
        ...
        if(self.comm_list!=nil){
            ...
            for (int i=0; i<[self.comm_list count]; i++) {
                self.comm_info = [self.comm_list objectAtIndex:i];
                if(FirstLoad){//第一次加载
                    if ([self.comm_info.is_recommend isEqualToString:@"是"]) {
                        [self.commlistArray addObject:self.comm_info];
                    }
