...
//wangyao 0107 change order_detail_address/phone
#import "OrderDetailAddress2TableViewCell.h"
#import "OrderDetailChangeContentTableViewCell.h"
#import "OrderDetailName2TableViewCell.h"
#import "OrderDetailPhoneNum2TableViewCell.h"
...
@property (strong,nonatomic) OrderDetailAddress2TableViewCell *ODAdd2Cell;
@property (strong,nonatomic) OrderDetailName2TableViewCell *ODName2Cell;
@property (strong,nonatomic) OrderDetailPhoneNum2TableViewCell *ODPhoneNum2Cell;
@property (strong,nonatomic) OrderDetailChangeContentTableViewCell *ODChangeContent2Cell;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0 ) {//收件人姓名
        self.ODName2Cell = [tableView dequeueReusableCellWithIdentifier:nil];
        
        if (!self.ODName2Cell) {//个人信息
            self.ODName2Cell= [[[NSBundle mainBundle]loadNibNamed:@"OrderDetailName2TableViewCell" owner:nil options:nil]objectAtIndex:0];
            self.ODName2Cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        self.ODName2Cell.OrderDetailName.text = self.cust_name;
//        self.ODAddCell.label_phone.text = self.cust_phone;
//        self.ODAddCell.label_address.text = self.cust_address;
               return self.ODName2Cell;
    }else if (indexPath.row ==1){//收件人电话
        self.ODPhoneNum2Cell = [tableView dequeueReusableCellWithIdentifier:nil];
        
        if (!self.ODPhoneNum2Cell) {
            self.ODPhoneNum2Cell= [[[NSBundle mainBundle]loadNibNamed:@"OrderDetailPhoneNum2TableViewCell" owner:nil options:nil]objectAtIndex:0];
            self.ODPhoneNum2Cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        self.ODPhoneNum2Cell.OrderDetailPhoneNum.text = self.cust_phone;

        return self.ODPhoneNum2Cell;
        
    }else if(indexPath.row == 2){//收件人地址
        self.ODAdd2Cell = [tableView dequeueReusableCellWithIdentifier:nil];
        
        if (!self.ODAdd2Cell) {//个人信息
            self.ODAdd2Cell= [[[NSBundle mainBundle]loadNibNamed:@"OrderDetailAddress2TableViewCell" owner:nil options:nil]objectAtIndex:0];
            self.ODAdd2Cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        self.ODAdd2Cell.OrderDetailAddress.text = self.cust_address;
        
        return self.ODAdd2Cell;

        
    }else if(indexPath.row == 3){//商品图片信息
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    OrderDetailChangeContentViewController *ODCCVC = [OrderDetailChangeContentViewController createFromStoryboardName:@"OrderDetailChangeContent" withIdentifier:@"order_detail_change_content"];
    switch (indexPath.row) {
        case 0:
            [ODCCVC getTitle:@"收货人姓名" andRow:0 andContent:_ODName2Cell.OrderDetailName.text];
            [self.navigationController pushViewController:ODCCVC animated:YES];
            

            break;
        case 1:
             [ODCCVC getTitle:@"收货人电话" andRow:1 andContent:_ODPhoneNum2Cell.OrderDetailPhoneNum.text];
            [self.navigationController pushViewController:ODCCVC animated:YES];

            break;
        case 2:
            [ODCCVC getTitle:@"收货人地址" andRow:2 andContent:_ODAdd2Cell.OrderDetailAddress.text];
            [self.navigationController pushViewController:ODCCVC animated:YES];
            break;
        default:
            break;
...
#pragma mark-----获取用户上一次·的·订单的真实姓名和地址 加电话------
-(void)getUserInfo{
    [StatusTool statusToolGetUserNameWithCommunityID:self.cust_community_id UserID:self.cust_id Success:^(id object) {
        ...
        self.cust_phone = item.user_phone;
       [self.table reloadData];
...
- (IBAction)SubmitOrderOnclick:(id)sender {
    ...
    [StatusTool statusToolAddNewOrderInfoWithShopID:self.shop_id CustID:self.cust_id CustName:self.ODName2Cell.OrderDetailName.text CustPhone:self.ODPhoneNum2Cell.OrderDetailPhoneNum.text OrderMoney:self.shop_order_money OrderSendfee:self.order_sendfee OrderState:self.order_state OrderAddress:self.ODAdd2Cell.OrderDetailAddress.text PayType:self.pay_type CommID:self.comm_id CommUnit:self.comm_unit CommPrice:self.comm_price CommName:self.comm_name BuyAmount:self.comm_buy_amount CommType:self.commodity_type_array OrderID:order_id Success:^(id object) {
...
//    switch ([defaults integerForKey:@"indexRow"]) {
//        case 0:
//            [self.ODName2Cell.OrderDetailName setText:[defaults objectForKey:@"ChangeContent"]];
//            NSLog(@"^^^^^^^^^CHANGECONTENT:%@",[defaults objectForKey:@"ChangeContent"]);
////            [self.table reloadData];
...
//           break;
//        case 1:
//            [self.ODPhoneNum2Cell.OrderDetailPhoneNum setText:[defaults objectForKey:@"ChangeContent"]];
//            [self.table reloadData];
//            break;
//        case 2:
//            [self.ODAdd2Cell.OrderDetailAddress setText:[defaults objectForKey:@"ChangeContent"]];
//            [self.table reloadData];
//            break;
//        default:
//            break;
            if([defaults integerForKey:@"indexRow"]==0){
                self.cust_name =[defaults objectForKey:@"ChangeContent"];
                NSLog(@"^^^^^^^^^CHANGECONTENT:%@",[defaults objectForKey:@"ChangeContent"]);
                //刷新单行
                NSIndexPath *indexPath=[NSIndexPath indexPathForRow:0 inSection:0];
                            [self.table reloadRowsAtIndexPaths:[NSArray arrayWithObjects:indexPath,nil] withRowAnimation:UITableViewRowAnimationNone];
                [defaults setInteger:999 forKey:@"indexRow"];
            }else if ([defaults integerForKey:@"indexRow"]==1){
                self.cust_phone =[defaults objectForKey:@"ChangeContent"];
                NSIndexPath *indexPath=[NSIndexPath indexPathForRow:1 inSection:0];
                [self.table reloadRowsAtIndexPaths:[NSArray arrayWithObjects:indexPath,nil] withRowAnimation:UITableViewRowAnimationNone];
                [defaults setInteger:999 forKey:@"indexRow"];
            }else if ([defaults integerForKey:@"indexRow"]==2){
                self.cust_address = [defaults objectForKey:@"ChangeContent"];
