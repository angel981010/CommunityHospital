...
//    [self.EstimateCell MakeBuyDate:estimate_info.buy_date];
    ...
    CGSize maxSize =CGSizeMake(CGFLOAT_MAX, 30);
//    CGSize nameSize = [self.EstimateCell.CustName.text sizeWithFont:self.EstimateCell.CustName.font constrainedToSize:maxSize lineBreakMode:NSLineBreakByClipping];
     CGSize nameSize = [self.EstimateCell.CustName.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} context:nil].size;
//     CGSize nameSize = [self.EstimateCell.CustName.text sizeWithFont:self.EstimateCell.CustName.font constrainedToSize:maxSize];
    CGRect rect = self.EstimateCell.CustName.frame;
    NSLog(@"^^^^^^^^^^namesize.width = %f",nameSize.width);
    if (nameSize.width>10) {
        nameSize.width = 10;
    }
    NSLog(@"^^^^^^^^^namesize.width_after = %f",nameSize.width);
    rect.size.width = nameSize.width;
    self.EstimateCell.CustName.frame = rect;
